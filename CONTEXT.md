...
**形成能悬浮读数 (formation-energy hover readout)**:
The floating panel that follows the pointer over a formation-energy plot and, at one Fermi level, lists every currently visible defect in descending formation energy (highest first, matching the chart's vertical reading). It shows the full list inside a capped, scrollable height and hides when the pointer leaves the plot (or its panel); on touch-only devices the same data is reached by tapping the plot, which pins a compact summary. It is an inspection surface, never an additional thermodynamic calculation — hiding defects from the panel never alters the intrinsic charge-neutrality Fermi level.
