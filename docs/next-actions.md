...
> 3. 新协议（仅重建两体系，其余 8 维持现状，CSV 带 protocol 列）：**晶格在超胞构建时一次定死（unitcell CONTCAR）；perfect 与缺陷一律 ISIF=2 固定该晶格；不再有 perfect ISIF=3 弛豫，不再做晶格同步（2026-08-17 策略，sync_lattice_from_perfect 已移除）**；ENCUT 按体系默认（encut null，不固定）；88 原子起步，门不过再升级 12 Å。
