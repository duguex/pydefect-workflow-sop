...
class modified_Outcar(Outcar):
    ...
    def read_force(self):
        forces = self.read_table_pattern(
            ...
            # row_pattern=r"\s*[0-9\-\.Ee]+\s+[0-9\-\.Ee]+\s+[0-9\-\.Ee]+\s+([0-9\-\.Ee]+)\s+([0-9\-\.Ee]+)\s+([0-9\-\.Ee]+)",
            footer_pattern=r"\s*\-+",
            postprocess=float,
            last_one_only=True,
        )
...
def read_zfs_path(path):
    ...
    # 优先读取CONTCAR，如果不存在则读取POSCAR并给出警告
    contcar_path = f"{path}/CONTCAR"
    poscar_path = f"{path}/POSCAR"
    
    if os.path.exists(contcar_path):
        pos = Structure.from_file(contcar_path)
    else:
        print(f"Warning: CONTCAR not found in {path}, using POSCAR instead")
        pos = Structure.from_file(poscar_path)
